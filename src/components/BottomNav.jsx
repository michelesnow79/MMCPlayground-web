...
const BottomNav = ({ onAddClick, showAddButton }) => {
    ...
    const isTabActive = (path) => {
        ...
        // Exception: If we are on a detail page but it was triggered from messages, keep messages active
        if (path === '/messages' && location.pathname.startsWith('/browse/') && location.state?.fromMessages) {
            return true;
        }
        // Normal browse highlight
        if (path === '/browse' && location.pathname.startsWith('/browse/') && !location.state?.fromMessages) {
            return true;
        }
        return false;
    };
    ...
    return (
        <nav className="bottom-nav">
            <NavLink to="/map" className={isTabActive('/map') ? "nav-item active" : "nav-item"}>
            ...
            <NavLink to="/browse" className={isTabActive('/browse') ? "nav-item active" : "nav-item"}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            ...
            <NavLink to="/messages" className={isTabActive('/messages') ? "nav-item active" : "nav-item"}>
            ...
            <NavLink to="/account" className={isTabActive('/account') ? "nav-item active" : "nav-item"}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
